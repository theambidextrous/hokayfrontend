...
import { getJobEditLink, editJob } from "../../api/api";
...
class SectionForm extends Component {
  ...
  componentWillMount() {
    // const newurl = window.location.pathname.split('/').slice(-1).pop();
    // console.log('child props ',newurl);
    const editLinkId = window.location.pathname.split('/').slice(-1).pop();
    this.setState({
        ...this.state,
        editLinkId:editLinkId,
    })
    getJobEditLink(editLinkId)
    .then( (res) => {
        if( res.status === 201 )
        {
            this.setState({
    ...
    .catch((err) => {
      this.setState({...this.state, isFound:false});
    });
  }
          ...
          <Container style={{minWidth:'100%'}}>
            <Row className="justify-content-center">
              <Col lg={12}>
                ...
                <h1 className="text-dark text-center hero-6-title font-weight-medium mb-4 line-height-1_4 hcustom">
                    Editing: <b>{this.state.title}</b> posted by <b>{this.state.organization}</b>
                </h1>
                ...
                <Row>
                    <Col lg={8} className="job-form">
                        ...
                        {/* badges */}
                        <Row className="form-inner" hidden={this.state.disableSome}>
                            <Col lg={12}><p className="text-center section-label">Design Your Job</p></Col>
                            <Col lg={12}>
                                <div className="form-group mb-2">
                                    <input disabled={this.state.disableSome} name="badge" type="checkbox" checked={this.state.showlogo} onChange={this.handleShowLogoChange} /><span className="badge-label">Show my ⭐️ company logo besides my job post(+${conf.job_cost_logo}) 
                                    <span className="benefit-badge">2x More views</span>
                                    <span className="rec-badge">Highly Recommended</span>
                                    </span>
                                </div>
                                <div className="form-group mb-2">
                                    <input disabled={this.state.disableSome} name="badge" type="checkbox" checked={this.state.matchJob} onChange={this.handleMatchJobChange} /><span className="badge-label">Get matched with suitable applicants from our <u>Network of 5000+ subscribers</u></span>
                                </div>
                                <div className="form-group mb-2">
                                    <input disabled={this.state.disableSome} name="badge" type="checkbox" checked={this.state.bumpTofront} onChange={this.handleBumpTofrontChange} /><span className="badge-label">Auto renew my job post and bump it to the frontpage after 30 days</span>
                                </div>
                                <div className="form-group mb-2">
                                    <input disabled={this.state.disableSome} name="badge" type="checkbox" onChange={this.handleShowInYellowChange} checked={this.state.showInYellow} /><span className="badge-label">
                                ...
                                <div className="form-group mb-2">
                                    <input disabled={this.state.disableSome} name="badge1" type="checkbox" onChange={this.handleShowInBrandChange} checked={this.state.showInBrand} /><span className="badge-label">
                                ...
                                <div className="form-group mb-2">
                                    <input disabled={this.state.disableSome} name="badge2" type="checkbox" onChange={this.handleStickyDayChange} checked={this.state.stickyDay} /><span className="badge-label">
                                        Sticky your post so it stays on 📎 top of the frontpage for ⏰ 24 hours (+${conf.job_cost_sticky_day})
                                        <span className="benefit-badge" title="Based on previous job post data compared to a regular job post">2x More views</span>
                                    </span>
                                </div>
                                <div className="form-group mb-2">
                                    <input disabled={this.state.disableSome} name="badge3" type="checkbox" onChange={this.handleStickyWeekChange} checked={this.state.stickyWeek} /><span className="badge-label">
                                ...
                                <div className="form-group mb-2">
                                    <input disabled={this.state.disableSome} name="badge" type="checkbox" onChange={this.handleStickyMonthChange} checked={this.state.stickyMonth} /><span className="badge-label">
                        ...
                        <Row className="form-inner">
                            ...
                            <Col lg={12}>
                                ...
                                <div className="form-group mb-4">
                                    ...
                                    <input readOnly={true} type="text" className="form-control input-ios" id="exampleFormControlInput1" value={this.state.coEmail} onChange={this.handleCoEmailChange} />
                                ...
                                <div className="form-group mb-4" hidden={this.state.disableSome}>
                                    <label htmlFor="exampleFormControlInput1" className="flabel">
                                        Company Card*
                                    </label>
                                    <Row>
            ...
            >
                <Row className="floating-foot">
                    ...
                    <Col lg={4}>
                        <button disabled={!stripe} type="submit" className="submit-job">
                        { this.state.disableSome ? (
                            <span>Save Changes - $0</span>
                        ):(
                            <span>Post Your Job - ${this.state.invAmount}</span>
                        )}
                        </button>
                    </Col>
                </Row>
